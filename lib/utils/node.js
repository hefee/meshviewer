define(['snabbdom', 'helper', 'moment'], function (V, helper, moment) {
  ...
  self.showDatabaseID = function showDatabaseID(d) {
    if (d.domain !== 'meshkit') {
      return undefined;
    }

    var nodeID = d.node_id.replace(/^0+/, '');
    return V.h('a', {
      props: {
        href: 'https://db.leipzig.freifunk.net/node/' + nodeID
      }
    }, nodeID);
  };
  ...
  self.showMACs = function showMACs(d) {
    var string = [];
    var macs = d.macs;
    if (!macs) {
      return undefined;
