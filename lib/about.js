define(function () {
  ...
  return function () {
    this.render = function render(d) {
      ...
      var community = '<h2>Communities</h2><ul>';
      config.domainNames.forEach(function (item) {
        if (item.web) {
          community += '<li><strong>' + item.name + '</strong> ' +
            '<a href="' + item.web + '" target="_blank">Web</a>';

          if (item.map) {
            community += ' | <a href="' + item.map + '" target="_blank">Map</a>';
          }

          community += '</li>';
        }
      });
      d.innerHTML += community + '</ul>';

      d.innerHTML += '<h3>AGPL 3</h3>' +
